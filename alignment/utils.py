...
def load_vectors(fname, params, source = bool, norm=True, center=False, verbose=True):
    
    txt_format = params.src_txt_format if source else params.tgt_txt_format
    no_words = params.no_words_src if source else params.no_words_tgt
    dim = params.dim
    maxload = params.max_load
    ...
    with io.open(fname, 'r', encoding='utf-8', newline='\n', errors='ignore') as f:
        ...
        for i, line in enumerate(f):
            if txt_format == "w2v":
                ...
                assert len(split) == 2
                word = line.split()[0]
                vect = np.array(line.split()[1].split(','), dtype = float)
                words.append(word)
                x[i,:] = vect
            else:
                word = line.split()[0]
                vect = np.array(line.split()[1:], dtype = float)
                words.append(word)
                x[i,:] = vect
    if norm:
...
def load_fasttext_vectors(fname, params, norm=True, center=False, verbose=True):
